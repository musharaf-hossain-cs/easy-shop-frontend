...
export default {
  ...
  created() {
    ...
    socket.emit('sendEmployeeByToken', this.$store.getters.getUser.token);

    socket.on('getEmployee',(res)=>{
      if(res.length>0){
        this.employee = res[0];
        if(this.employee.JOB_TITLE.toLowerCase()==='delivery boy'){
          this.$store.dispatch('setDeliveryBoy',true);
        }else{
          this.$store.dispatch('setDeliveryBoy',false);
        }
        this.valid = true;
        let root = this;
